...
Para criar uma nova branch, primeiro execute um `pull`, para garantir que os arquivos estão atualizados. Então, execute `git checkout -b [Nome da branch]`. O modificador `-b` serve para criar uma nova branch. Agora, você está em uma branch diferente da master, podendo fazer mudanças e commits sem alterar o código original. Para mandar essa branch para o GitHub e criar uma branch remota, usamos o comando `git push origin -u [Nome do branch]`. Depois da primeira vez, podemos usar o push direto(desde que estejamos na branch certa).
